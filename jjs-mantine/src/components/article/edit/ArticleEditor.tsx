// src/components/article/edit/ArticleEditor.tsx
...
const DEFAULT_POST: Partial<PostDetail> = {
  title: "", previewText: "", body: "", releaseDate: "", expireDate: "",
  commentsEnabled: false, approved: false, viewCount: 0, imageUrl: "", href: "",
  categoryIds: [], categories: [],
};

export default function ArticleEditor({ post, categories = [], isSaving = false, onSave, onCancel }: ArticleEditorProps) {
  const form = useForm<PostDetail>({
    mode: 'uncontrolled',
    initialValues: DEFAULT_POST as PostDetail,
    validate: zodResolver(articleSchema),
  });

  useEffect(() => {
    if (post) {
      form.initialize(post);
    }
  }, [post]);
