...
if exists(select * from information_schema.columns where table_name = 'Posts' and column_name = 'Body'and data_type = 'ntext') begin
   ...
   -- select postid, PreviewText, Title, dbo.[ConvertHtmlToMarkdown]([Body]) from Posts
   update [Posts] 
   set [PreviewText] = dbo.[ConvertHtmlToMarkdown]([PreviewText])
      ,[Body] = dbo.[ConvertHtmlToMarkdown]([Body])
   ;
end
...
set 
Title = 'Test Markdown'
   ,PreviewText = 'Testing a bundle of markdown syntax here **testing!**'
   ,Body = '# Welcome to StackEdit!
